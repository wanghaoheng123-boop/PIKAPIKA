...
- **CLI CI:** `cd Packages/PikaCoreBase && swift test` — exercises domain logic without SwiftData. **`PikaAI`** depends on **`PikaCoreBase` only** so `cd Packages/PikaAI && swift test` also avoids SwiftData (matches GitHub **Pika CI**).
